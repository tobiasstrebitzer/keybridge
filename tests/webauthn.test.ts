...
// homedir() at import time. Redirect HOME BEFORE importing the modules (static
// ESM imports are hoisted, so these must be dynamic), or the test would write
// into the real store.
const realHome = homedir()
process.env.HOME = mkdtempSync(join(tmpdir(), 'kb-webauthn-'))
...
type CoseMap = Map<number, Buffer>
...
function parseAuthData (buf: Buffer) {
  ...
  let credId: Buffer | undefined, cose: CoseMap | undefined
  if (flags & 0x40) { // AT: attested credential data present
    ...
    cose = decode(buf.subarray(55 + credIdLen)) as CoseMap
  }
  return { rpIdHash, flags, signCount, credId, cose }
}

function coseToPublicKey (cose: CoseMap): KeyObject {
...
test('registration → authentication produces an RP-verifiable ES256 assertion', async () => {
  ...
  const regCdj = JSON.parse(Buffer.from(cred.response.clientDataJSON as string, 'base64url').toString('utf8'))
  ...
  const att = decode(Buffer.from(cred.response.attestationObject as string, 'base64url')) as Map<string, Buffer>
  ...
  const asr = await handleGet({
    ...
    rpId,
    allowCredentials: [{ id: cred.rawId }],
  }, origin, signer)

  const getCdj = JSON.parse(Buffer.from(asr.response.clientDataJSON as string, 'base64url').toString('utf8'))
  ...
  const signature = Buffer.from(asr.response.signature as string, 'base64url')
...
test('a tampered challenge breaks verification (sanity: we are really signing)', async () => {
  ...
  const ok = createVerify('SHA256').update(forged).verify(publicKey, Buffer.from(asr.response.signature as string, 'base64url'))
...
test('assertion for an unknown rpId is refused with the ENOCRED fallback signal', async () => {
  ...
  await assert.rejects(
    ...
    (e: Error & { code?: string }) => /no keybridge credential/.test(e.message) && e.code === 'ENOCRED',
  )
})
